...
from dataset import DUETPreData
from trainer import Pretrain_Train
...
def main():
    ...
    parser.add_argument("--model_name", default='DUET-pre', type=str)
    ...
    # fairness group
    parser.add_argument("--pre_train_sample", default=4, type=int, help="pre_train sample number")
    parser.add_argument("--hyper_train_sample", default=4, type=int, help="hyper_train sample number")
    parser.add_argument("--base_train_sample", default=4, type=int, help="base_train sample number")
    ...
    train_data = pickle.load(open(args.data_dir + 'pre_train.pkl', 'rb'))
    valid_data = pickle.load(open(args.data_dir + 'pre_valid.pkl', 'rb'))
    ...
    train_dataset = DUETPreData(args, train_data, data_type='train')
    ...
    eval_dataset = DUETPreData(args, valid_data, test_neg_items=sample_seq, data_type='valid')
    eval_sampler = SequentialSampler(eval_dataset)
    eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.batch_size)

    trainer = Pretrain_Train(model, args)

    if args.do_eval:
        trainer.load(args.checkpoint_path)
        print(f'Load model from {args.checkpoint_path} for test!')
        scores, result_info = trainer.eval_stage(0, eval_dataloader, full_sort=False, test=False)
